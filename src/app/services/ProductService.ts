...
export class ProductService {
  ...
  private cartSignal = signal<IProduct[]>([]);
  private favoritesSignal = signal<IProduct[]>([]);
  http = inject(HttpClient);
  getProducts() {
    return this.http.get(`${this.apiUrl}?limit=10&skip=20`);
  }
  ...
  getFavorites() {
    console.log('getFavouritest Raised', this.favoritesSignal());
  ...
  addToCart(product: IProduct) {
    ...
    if (!exists) {
      this.cartSignal.set([...current, product]);
      console.log('ADD TO CART RAISED ', this.cartSignal().length);
    } else {
      console.warn('Product already in Cart}:', product.id);
    }
  }
  addToFavorites(product: IProduct) {
    ...
    if (!exists) {
      ...
      console.log('Product id:', product.id, 'Added To Fav');
    } else {
      console.warn('Product already in favorites:', product.id);
    }
  }
  ...
  removeFromFavorites(productId: number) {
    this.favoritesSignal.set(this.favoritesSignal().filter((p) => p.id !== productId));
  }
